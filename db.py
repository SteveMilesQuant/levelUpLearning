...
def get_db(app: FastAPI):
    if app.db is None:
        try:
            app.db = sqlite3.connect('file:{}?mode=rw'.format(pathname2url(app.config.db_path)), check_same_thread=False)
        except sqlite3.OperationalError:
            app.db = sqlite3.connect(app.config.db_path, check_same_thread=False)
